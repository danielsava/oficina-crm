...
- **OpenAPI / Swagger UI**:
  - The extension `quarkus-smallrye-openapi` is enabled. The OpenAPI contract is published at `/q/openapi` (JSON; `?format=yaml` for YAML) and the Swagger UI at `/q/swagger-ui`. Even without explicit annotations, SmallRye OpenAPI generates a working contract from JAX-RS annotations, return types and DTO records — that baseline is intentionally what we rely on.
  ...
  - **OpenAPI documentation annotations are OFF by default**. Do NOT add `@Tag`, `@Operation`, `@APIResponse`, `@Parameter` or `@Schema` to REST classes, methods or DTO records as a routine practice. These annotations duplicate information already inferable from the code (method name, return type, record components, JAX-RS path/verb) and have polluted the codebase in past iterations. See [ADR-0010](doc/adr/0010-anotacoes-openapi-e-javadoc-sob-demanda.md).
  - These annotations MUST be added **only when explicitly requested by the user**, and only on the specific class/method/field for which the user asks. Do not retrofit them across the codebase or "improve documentation" on adjacent endpoints without an explicit request.
  - For error responses, the contract relies on the standard codes (400, 404, 409, 500) returning `application/problem+json` per RFC 7807 (see ADR-0004). The mappers in `infra/exception/*` produce the correct media type and status; explicit `@APIResponse` declarations are not required.
  - **Frontend type generation** — the `/q/openapi` contract remains the **canonical source** for TypeScript types in `frontend-ultima` (via `openapi-typescript` or equivalent). When changing a DTO, treat the impact on the frontend's generated types as part of the change set. See ADR-0006 and the corresponding section in `frontend-ultima/AGENTS.md`.
...
- **Standard Layers**: Each entity should follow a consistent layer pattern:
    - `Entity`: Extending `common.base.BaseEntity`.
    - `Repository`: Interface extending `common.base.BaseRepository<Entity>`.
    - `Service`: Class extending `common.base.BaseService<Entity, EditDTO, ListDTO>`.
    - `Rest`: Class extending `common.base.BaseRest<Entity, EditDTO, ListDTO>`.
    - `DTOs`: Specifically `EditDTO` for creation/updates and `ListDTO` for listings.
    - `Mapper`: MapStruct interface extending `common.base.BaseMapper<Entity, EditDTO>`.

## Common Base Components

Base classes live under `src/main/java/common/`, organized in three sub-packages:

- `common.base` — generic CRUD foundations (`BaseEntity`, `BaseRepository`, `BaseService`, `BaseRest`, `BaseMapper`) plus `EnumStatusEntity`.
- `common.paginacao` — paginated response envelope (`Pagina`) and sort parsing (`SortCriterio`, `SortDirecao`, `SortParser`).
- `common.filtro` — `POST /buscar` contract (`FiltroDTO`, `CriterioFiltro`, `OperadorFiltro`, `OperadorLogico`) and the JPQL builder (`FiltroAvancadoQueryBuilder`).

Note: `DbSchemas` is **not** part of `common` — it lives in `infra.persistence` because the `common.*` packages are expected to be extracted into a reusable JAR, and schema constants are application-specific infrastructure.
...
- **Pagina**: Generic record `common.paginacao.Pagina<T>` is the standard response envelope for paginated listings (fields: `content`, `page`, `size`, `totalElements`, `totalPages`). Name in Portuguese is deliberate to avoid clashing with `io.quarkus.panache.common.Page`. See [ADR-0009](doc/adr/0009-paginacao-ordenacao-filtros-no-baserest.md).
...
- **Statuses**: Use `common.base.EnumStatusEntity` (`ATIVO`, `INATIVO`) for soft deletes/record status.
...
- **Keep code clean and readable; do NOT add Javadoc, block comments or inline narration as a routine practice.** Past iterations accumulated documentation that duplicated method names, restated obvious contracts and pushed the actual logic out of the first screen — the resulting noise hurt readability more than it helped. See [ADR-0010](doc/adr/0010-anotacoes-openapi-e-javadoc-sob-demanda.md).
- Add Javadoc or comments **only when explicitly requested by the user**, OR when a non-obvious decision needs to be preserved against future editors silently breaking it (e.g., "this filter is always combined with AND regardless of the client's operadorLogico"). When in doubt, leave it out.
- When a comment is genuinely necessary, prefer a **short inline comment** (one or two lines) at the exact point of the decision, instead of a Javadoc block at the top of the class/method.
- This rule applies to ALL Java sources under `src/main/java/**` (classes, records, enums, interfaces, methods, fields). Test sources may carry brief `@DisplayName` text but follow the same restraint for narrative comments.
- When refactoring existing code, **strip existing Javadoc/comments that do not meet the bar above** — do not preserve documentation just because it was there.
