...
- **Sort/filtro por campo de relacionamento** (ex.: `cliente.nome`): não há suporte direto via dot notation no contrato HTTP. A forma idiomática neste desenho é **expor o campo achatado no `ListDTO`** (ex.: componente `String clienteNome` no `OrdemServicoListDTO`), com a projeção do `*Service` populando-o a partir de `OrdemServico.cliente.getNome()` (via MapStruct, JPQL `SELECT NEW` ou ajuste do método `buscarAvancado` para usar um JPQL custom). Uma vez no `ListDTO`, o campo vira automaticamente filtrável e ordenável como qualquer outro (whitelist deriva do `ListDTO`, ver decisão 3). Isto **não é** limitação técnica do JPQL/Panache (JPQL suporta `ORDER BY u.cliente.nome` nativamente, gerando o `JOIN` apropriado) — é consequência consciente do princípio "o que está no `ListDTO` é o que pode ser filtrado/ordenado". O custo é localizado no `*Service` da entidade que precisa do campo, não no `BaseService` genérico, mantendo o desenho do framework limpo. Caso surja necessidade frequente de expor relacionamentos sem achatar no DTO, abrir caso de uso específico — pode justificar revisão futura do desenho (ex.: introduzir mapeamento `componente DTO → path JPQL` sobrescritível por `*Service`).
